...
if __name__ == "__main__":
    ...
    if len(tp_df) > 0 and len(tn_df) > 0:
        ...
        #r_df = df[df['Set']=='R only']
        #nr_df = df[df['Set']=='NR only']
        #r_nr_df = df[df['Set']=='R & NR']
        #plot_catplot(r_df, x_str="Enrichment Score", y_str="Pathway", 
        #             ttl_str="Enrichment of responder specific pathways", fig_str=args.output_path+".R-only")
        #plot_catplot(nr_df, x_str="Enrichment Score", y_str="Pathway",
        #             ttl_str="Enrichment of responder specific pathways", fig_str=args.output_path+".NR-only")
        #plot_catplot(r_nr_df, x_str="Enrichment Score", y_str="Pathway",
        #             ttl_str="Enrichment of responder specific pathways", fig_str=args.output_path+".R-NR")
