...
def create_classifier(n_input, param_dict):
    """
...
def create_regressor(n_input, param_dict):
    """
    :param n_input: integer representing the number of features
    :param param_dict: dictionary of hyperparameters
    return model
...
def run_cv(train_df, valid_df, test_df, cv_str=None, task_str='classification', param_dict=None, fout_str="./"):
    """
    ...
    # create a model 
    if task_str == 'classification':
        model = create_classifier(n_input, param_dict)
    else:
        model = create_regressor(n_input, param_dict)
...
if __name__ == "__main__":
    ...
    if args.param_path is None:
        ...
        # convert types
        param_dict['batch_size'] = round(float(param_dict['batch_size']))
        param_dict['dropout_float'] = float(param_dict['dropout_float'])
        param_dict['earlyStop'] = round(float(param_dict['earlyStop']))
        param_dict['epoch'] = round(float(param_dict['epoch']))
        param_dict['learning_rate'] = float(param_dict['learning_rate'])
        param_dict['n_layer'] = round(float(param_dict['n_layer']))
        param_dict['n_neuron'] = round(float(param_dict['n_neuron']))
        param_dict['retrain'] = eval(param_dict['retrain'])
        #print(param_dict)

    else:
        param_df = pd.read_csv(args.param_path, header=0, index_col=0, sep="\t")
        key = param_df.columns.tolist()[0]
        param_dict = param_df.to_dict()[key]
        #print(param_dict)
        # convert types
        param_dict['batch_size'] = round(float(param_dict['batch_size']))
        param_dict['dropout_float'] = float(param_dict['dropout_float'])
    ...
    for i in range(0, args.cv_int):
        ...
        pred_df = run_cv(scaled_train_df, scaled_valid_df, scaled_test_df, cv_str=cv_str, task_str=args.task_str,
                         param_dict=param_dict, fout_str=args.output_path)
